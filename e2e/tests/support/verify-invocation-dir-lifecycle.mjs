...
// real Playwright run, meaning globalTeardown was not actually removing the
...
// createInvocationDir()'s worker-process dedup and globalTeardown's cleanup
...
console.log("PASS: no invocation directories leaked (createInvocationDir dedup + globalTeardown cleanup both held).");

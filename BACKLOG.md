...
> F008（Workflow Template Admin & Runtime Health）已于 2026-08-09 完成全部 AC 验收与门禁（lint / format:check / typecheck / test / build），schema v10（`admin_audit_events` + `workflow_templates` 两个唯一索引）已按落地顺序应用；详情见 `docs/features/0.2/F008-workflow-template-admin-runtime-health/spec.md`。

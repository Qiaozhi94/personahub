...
> F013 shipped on 2026-09-14 — implementation review converged, `npm run verify:release` and Windows CI are green, so it is `done` and has left the active table; F012 moved to `ready-for-development` on 2026-09-14 after its pre-development review (cycle 24, 2 rounds) and into `in-progress` on 2026-09-15 (T000–T024 implemented on branch `f012-session-dispatch`; T025 old-write-path removal and T026 release gate remain); F011 and F014 remain in their own review and development stages. The former Room/Squad plan has been replaced by the trusted task workbench
...
| F012 | 0.3     | Session, Dispatch & Intervention | in-progress | unassigned | `docs/features/0.3/F012-session-dispatch-intervention/spec.md` |

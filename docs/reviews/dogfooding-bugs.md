...
> **关单纪律（2026-09-03 起由门禁强制）**：
> - `fixed` 必须同时有**非 `—` 的回归测试**与**修复 commit**。修不修得好由复验说了算，
>   一句「修了」不算关单——借自 clowder-ai F266「closure 只能来自复验通过 / 明确裁决 / sunset」。
> - 合法状态增加 `wontfix`：看过了、决定不修。它必须在详情块里写 **`不修理由`**，且不得带修复
>   commit。不修是一个决定，决定要留理由；它也仍然计入统计，不会静默消失。
> - `open` 仍然不得带修复 commit。
>
> 统计/校验/列 open 用：`npm run bug:log`（已接入 `npm run verify`，不再只靠人手动跑）。
...
- **备注**：这是同一缺陷家族的**第三例**（BUG-004 宿主装了什么 + 什么平台、BUG-005 什么身份、
  ...
  <a href="self-test-system-plan.md">`self-test-system-plan.md`</a> **§7.5**——初版标题为
  「宿主无关」，含 5 条具体规则与跳过的边界判据，并诚实登记了"暂无自动门禁、靠纪律维持"
  这一状态；BUG-007 出现后判据已扩写为「断言只钉自己拥有的不变量，不钉偶然事实」。
  修复后 `npm run verify` 首次全绿（exit 0）。
...
- **备注**：这是「断言钉在偶然事实上」家族的第四例，前三例是 BUG-004/005/006
  （宿主 PATH / uid / 平台）。本例钉的不是宿主，是版本号与实现细节名，
  <a href="self-test-system-plan.md">`self-test-system-plan.md`</a> §7.5 初版措辞
  （「不得依赖宿主 PATH、uid 或运行平台」）覆盖不到它。**已按裁决把 §7.5 的判据从
  「宿主无关」扩写为「断言只钉自己拥有的不变量，不钉偶然事实」**，并新增第 6 条
  （版本号与命名）与第 7 条（自检问法）。

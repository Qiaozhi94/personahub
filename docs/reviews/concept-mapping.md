...
>
> **配套文件（2026-09-03）**：本文管**概念与页面形态**；clowder 的**治理机制**
> （评估怎么立、结论怎么关单、能力怎么晋升）核查另见
> <a href="clowder-governance-borrowing.md">`clowder-governance-borrowing.md`</a>。
...
**布局结论**：右栏只放快照，clowder 的固定堆叠形态**够用，可直接照搬**。
（早前「右栏需随状态重排布局」的判断基于「右栏要承载主操作」，该前提已被 §5.2 推翻。）

**与 PRD §13「右侧 Inspector 分 tab」的关系（2026-08-15 澄清）**：本节说的「照搬固定堆叠」
是形态素材层的判断（拼装优先级链第 4 层），不构成对分区方式的决定。PRD §13 是硬约束，
位于第 2 层，**上层否决下层**。因此是否分 tab 取决于最终区块数量与滚动长度，
由 **M4-T02 裁决并记录**；`personahub-user-journeys.md` §6.4 已相应移除形态断言，
只保留「右栏承载快照、不承载动作」这条行为规则。

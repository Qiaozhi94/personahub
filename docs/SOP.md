...
| 日期 | 来源提交 | 修订目的 | 修订内容 |
| --- | --- | --- | --- |
| 2026-09-11 | （本次修订） | F009 review 的 T031 人工走查把"干净库首屏对不对""96 条 deferred 有没有入口"整体标成人工，实际两者都是结构事实、可以自动化；讨论中确认需要把"这能不能写成断言"固化成通用判断标准，而不是每次临场判断 | 新增"自动化与人工判断边界纪律"：结构/行为事实必须转自动化门禁，不得留在人工清单里；只有真正的审美/措辞质量判断才留人工，且要写明原因 |
...
- **判断标准只有一条：这一步验证的是"结构/行为事实"还是"审美/措辞质量"。** 前者——元素在不在、路由能不能到、状态变没变、文案是否命中禁用词表、请求发没发、返回码对不对——永远能写成 Playwright / vitest 断言，写不出来就是没找对断言方式，不是"这类东西天生测不了"。后者——分组好不好看、措辞顺不顺、这版设计是否合理——是真实的审美/语感判断，自动化在这里只能证明"跟上次比没变"（视觉回归），证明不了"这版好不好"，而且第一次基准还是要人批准。**不要把后者伪装成前者去写断言，也不要把前者当成后者塞进人工清单里不做。**
- **人工检查清单里的每一项，先问一遍属于哪一类；属于前者的必须转成自动化门禁，不允许留在"人工走查"里长期存在。** 反面案例：F009 review R1-005/R1-011 的 T031 曾把"干净数据库首屏对不对""96 条 deferred 是否确实没有入口"整体标成人工——前者是"零 seed 库首次打开时该出现的空态组件在不在"，后者是"点不点得到某个还没做的功能"，两个都是结构事实，不是审美判断。补上专门跑空库的 Playwright 配置（`e2e/playwright.empty-db.config.ts`）和一个把 96 条按"背后路由/导航槽位是否已被现有门禁证明不可达"分桶、锁定分桶数量的门禁（`tools/check-v03-plan-contracts.test.mjs::F009-CODE-DEFERRED-INVENTORY`）后，96 条里 64 条从"人工看一眼"变成"只要两个已有测试不被削弱就有效"，只剩 32 条是已启用页面上的具体元素、需要针对该页面单独写断言——过程与结果见 `docs/reviews/journey-test-matrix.md` §5.1。
- **不能转成自动化的部分（分组是否好看、措辞是否得体等）留在人工清单里，但要写清楚"这是不可替代的审美判断"，不要笼统地写"人工复核"掩盖其中其实可自动化的部分。** 目的是让人工走查清单只剩真正需要人的东西，而不是因为"图省事没去想能不能自动化"而越堆越大。

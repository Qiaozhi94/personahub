...
| 口径 | 是什么 | 归属 |
| --- | --- | --- |
| **额度** | 约束。当前还可派出几次 high | Runtime（ADR 0012 第 3 条），不进统计模块 |
...
统计模块只回答「过去这段时间发生了什么」，全屏不出现实时数字。额度回到它的三个消费点：状态栏提醒、派工弹层的预估行、设置 · 运行时的配置（那张组合表本来就有额度列；V3.21 起运行时不再是一级面，见 design.md §3.5.3）。「因额度不足未执行」的记录留在自动化面，与「已执行但失败」分开记录的口径不变。

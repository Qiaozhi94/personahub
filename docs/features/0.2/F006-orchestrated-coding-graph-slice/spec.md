...
- **Q3**（已关闭 → `design.md` 第 6 节，2026-08-02 修正）：Edge 只承载引用，不承载内容。**引用指向新增的 `graph.node_result` 事件**，该事件是节点结果的唯一真相源，且必须加入 `EvidenceService` 的可信 payload allowlist；取用一律经 `resolveTrustedPayload()` 且 scope 不带 `runId`。截断由本 slice 自行实现（既有 `EvidenceResolution` 从不产出 `truncated`）。共 **8 个** `graph.*` ThreadEvent（`node_queued` / `node_result` / `node_completed` / `edge_traversed` / `join_satisfied` / `executor_reassigned` / `blocked` / `terminal`；`blocked` 与 `terminal` 分开，因为 `blocked` 是可恢复的非终态），Edge 定义按 `GraphEdgeV1` 显式声明 outcome/join/载荷槽位。

...
- [ ] T013：`shared/src/types/` 新增 `GraphRunStatus`、`NodeRunStatus`、`GraphBlockReason`、`GraphNodeKey` 与 DTO。
...
- [ ] T020：`server/src/runtime/graph/definitions.ts`：内置 `wgd_coding_dual_review` v1，声明三节点与两条边（`joinPolicy: all_required`）。
...
- [ ] T030：节点结果 envelope 解析器（复用 F004 的容错解析手法，不复用 validator 语义）；解析失败 → NodeRun `failed` + `result_unparsable`。
...
- [ ] T033：synthesis 节点上下文装配——按两条入边的 `result_event_id` 取前驱结果，走 `evidence_refs` / `EvidenceResolution`，`truncated` 状态必须透传给节点而不是静默缩水。
...
- [ ] T040：`GraphRecoveryService.reconcile()` 五步，按 `design.md` 第 7 节实现。
...
- [ ] T046：节点级 retry API + `GraphBlockReason` 独立恢复入口；断言**不复用** validation unblock（`recovery-action.ts` 的守卫必须仍然拒绝图类 blocker）。
- [ ] T047：escalation 路径测试——escalation 致 Run 失败 → NodeRun `failed` → GraphRun `blocked`。
...
- [ ] T051：HTTP 路由 `GET /api/issues/:id/graph`、`POST /api/graph-runs/:id/nodes/:key/retry`，沿用项目既定的 zod 边界校验（F005 已统一该做法）。

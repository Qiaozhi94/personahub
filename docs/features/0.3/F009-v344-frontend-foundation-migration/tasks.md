...
- [ ] T004 (`UX-002`, `NFR-002`): 实现 shell 级 `TaskDraftStore` 与 `task:${taskId}:composer` key/revision contract；覆盖跨 task/project/surface 往返、后续 view 共用 key、成功/失败/迟到响应、显式丢弃、not-found 清理和刷新前 `beforeunload` 提示，并做一次把清理条件放宽为“任意成功响应”的失败变异。 — verify: `npm test --workspace web`

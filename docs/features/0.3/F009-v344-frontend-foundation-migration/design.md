...
以新的 application shell、route registry、共享交互原语和 legacy view adapter 替换当前单页三栏拼装。既有 hooks / API client 和领域命令优先复用；旧页面拆成事实读取、动作入口和视觉容器三类，前两类按迁移矩阵接入，旧视觉容器退役。
...
先落全局 token、字体层级、导航、主栏 / 副栏框架和 loading / empty / error / partial 组件，再迁移项目、任务、执行事实、证据及管理入口。未交付工作面不注册；确需保留稳定槽位时展示明确不可用原因。开发态提供 route / surface 清单和 legacy adapter 计数，生产界面不出现设计过程术语。
...
确认 F009 是 v0.3 首个实施 Feature；它迁移现有能力，不等待 F010–F013 的新模型，也不承诺设计稿九个工作面全部上线。兼容 adapter 必须按 F011–F013 接管关系拆分。残余风险是视觉迁移扩大为业务重写，以“数据库零变更、canonical API 不变”和迁移矩阵作为硬边界。

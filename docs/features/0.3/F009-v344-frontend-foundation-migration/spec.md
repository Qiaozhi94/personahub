...
- [x] **AC-005** (`FR-006`, `NFR-003`): 新前端写操作全部命中既有 canonical API，兼容 adapter 有独立清单、替换 owner 和删除条件。—— 测试 [任务级]：`web/src/f009-pages.test.tsx`（T010，A001–A005 写入命中既有 API）、`web/src/f009-runtime-settings.test.tsx`（T013）与 T020 canonical API 静态断言（`tools/check-v03-plan-contracts.test.mjs`）；测试 [需求级]：`e2e/tests/f009-create-task.spec.ts`「BC-056」（T022，确认前零写 / 重复幂等）、`e2e/tests/f009-runtime-adapters.spec.ts`「BC-053」（adapter 配置唯一写入口）。 - tests: `web/src/f009-pages.test.tsx` `web/src/f009-runtime-settings.test.tsx` `tools/check-v03-plan-contracts.test.mjs` `e2e/tests/f009-create-task.spec.ts` `e2e/tests/f009-runtime-adapters.spec.ts`

...
- [ ] T003 (`FR-001`, `FR-002`, `FR-006`): 聚合 overview，并按 spec 11 行命名矩阵实现状态 projection；每行固定优先级、主操作、恢复与保留事实。 — verify: `npm test --workspace server`
...
- [ ] T020 (`AC-001`, `AC-002`, `AC-005`): 建立 11 个具名 Task state fixture、可信度与验收写链组件 / API fixture；逐行断言首屏 / 动作 / 恢复 / 保留事实，故障注入摘要写入并验证不会产生无摘要 done。 — verify: `npm test`

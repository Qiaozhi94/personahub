...
- **FR-004**：对 F009 之后已发布的 canonical deep links 做跨 schema 守恒；更早版本只处理有 release / source 证据的 URL inventory（当前仅 `/`），无证据的对象深链不猜测、不宣称迁移。
...
- [ ] **AC-002** (`FR-003`, `FR-004`, `NFR-001`): 所有发布 schema fixture 升级、F009+ canonical deep link 守恒和有证据 legacy URL 报告通过；新格式的刷新恢复与未知 ID 行为不漂移。

...
- `case_insensitive INTEGER`：授权时对该仓库根实测的大小写行为（探针文件写入后以变体大小写读取）；NULL 表示未测出，比较时按**敏感**处理。不按平台名假定——APFS 可格式化为大小写敏感卷。
...
5. 大小写：**存原文**。比较语义**不按平台名假定**——APFS 可以格式化成大小写敏感卷，把所有 macOS 当不敏感会在这类卷上错误地扩大授权范围。改为：Windows 按不敏感；其余平台**默认敏感**（保守侧），仅当授权时对该仓库根实测过大小写行为（写入探针文件再以变体大小写读取）并记录在 `repository_machine_paths` 的 `case_insensitive INTEGER` 列时，才按实测值比较。探测失败按敏感处理。
...
| `AC-002` | unit               | `server/tests/unit/scope-validation.test.ts`                                                        | **`""` / `"."` / `"/"` 三个整仓别名在拒绝规则之前归一成 `""`**（证明第 0 步先于第 1 步执行），而 `"/src"` 仍按绝对路径拒绝；其余五类非法前缀逐一被拒并返回 `SCOPE_INVALID_PREFIX`——`C:\x` 盘符、`\\server\share` UNC、`../..` 逃逸、含 NUL / 控制字符、含反斜杠；同值与被包含前缀去重保留较短者；**大小写：Windows 按不敏感，其余平台默认敏感，仅在 `case_insensitive` 列有实测值时按实测比较**（构造一个大小写敏感卷的 fixture 断言不会错误放宽）；`write` 不在 `read` 内时返回 `SCOPE_WRITE_NOT_IN_READ`                                   |

...
| `AC-002` | unit + integration | `server/tests/unit/repository-path.test.ts`、`server/tests/integration/repository-registry.test.ts` | symlink / junction 越界拒绝、大小写路径、`path.relative` 边界（`/a/bc` 不在 `/a/b` 内、`src/ab` 不在 `src/a` 内）、参考仓库 `read_write` 硬拒绝、项目范围只能收紧、旧 workspace 迁移不产生已授权 `real_path` **`role='reference'` 行写 `access='read_write'` 被 CHECK 拒绝**（不是应用层拦截）；项目级与机器级 access 取较严者；同一 Repository 被两个项目分别以 read_write / read_only 引用时互不影响                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |

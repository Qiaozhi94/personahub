...
校验规则：`status_map` 三个键（`satisfied` / `failed` / `not_applicable`）必须齐全；取值必须落在 `resolved | missing | truncated` 内，越域拒绝激活（`SKILL_EVIDENCE_STATUS_UNKNOWN`）；同一状态不得出现在两个键下（互斥）。**三个状态必须被完整覆盖**：`resolved` / `missing` / `truncated` 各自恰好出现在一个键下，未覆盖即报 `SKILL_EVIDENCE_STATUS_UNMAPPED` 并**拒绝激活**。上一轮同一句里既要求完整覆盖、又描述"未映射时按 failed 处理并告警"，两者互斥——完整覆盖成立则不存在未映射状态，该分支永远走不到。这里取前者：缺映射是配置错误，应当在激活时挡下，而不是静默按 `failed` 放行。带 `evidence_kind: "artifact"` 的 requirement 一律报 `SKILL_EVIDENCE_KIND_UNAVAILABLE`。
...
| `AC-004` | unit               | `server/tests/unit/evidence-spec.test.ts`                                                           | `status_map` 取值必须落在既有 `EvidenceResolution.status`（`resolved`/`missing`/`truncated`）内，越域即 `SKILL_EVIDENCE_STATUS_UNKNOWN`（含误用的 `emitted`/`present`/`empty` 作为负例）；三键齐全互斥；未完整覆盖时发 `SKILL_EVIDENCE_STATUS_UNMAPPED` 且按 `failed` 判定；**`evidence_kind: "artifact"` 一律报 `SKILL_EVIDENCE_KIND_UNAVAILABLE`**（v0.3 不含该 kind，`EvidenceRefKind` 里也没有它）；registry 的 summary/preview/open 由 kind 决定，要求侧不可覆写                                                                                                                                                                                                                                                                                                                                                                                                                                             |

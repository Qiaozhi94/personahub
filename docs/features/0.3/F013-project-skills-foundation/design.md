...
生命周期：`create` 写入新行（首个 Space 同时置 `is_default=1, is_selected=1`），并在**同一事务**内为所有全局 Skill 物化 `skill_space_state`（见 §3 冲突消解闭环）；`select` 在单事务内清旧选中、置新选中，目标必须 `state='active'`；`archive` 要求目标非当前选中且非 default，否则返回 `SPACE_ARCHIVE_BLOCKED`——**默认 Space 永不可归档**，因为历史数据以它为归属根；`restore` 把 `archived` 改回 `active`，不自动选中。v0.3 不支持物理删除（spec §5）。
...
- **删除保护**：引用方必须**按 schema 实际枚举**，不能只数新表。对 `projects(id)` 持外键的现有表是 `workspaces`（`schema-v1.ts`）、`issues`（`schema-v1.ts`）、`agent_configs`（`schema-v2.ts`）、`intake_confirmations`（`schema-v9.ts`），本 Feature 再加 `project_repository_refs` 与 `project_skill_refs`——**共六类**。`DELETE /api/projects/:id` 逐类检查，任一非空即返回 `PROJECT_HAS_REFERENCES` 并列出阻塞类别；六类全空才允许删除。v0.3 不提供级联删除。
  - 上一轮只检查三类（Issue / 仓库引用 / Skill 引用），漏掉的三张表都带真实外键：删除会被 SQLite 直接拒绝，用户看到的是底层约束错误而不是可读的阻塞清单。
  - 该清单随 schema 演进，**新增任何指向 `projects(id)` 的外键都必须同步这里**；验收测试用 `PRAGMA foreign_key_list` 反查全部引用表并与检查清单比对，清单漏项即测试失败。
...
- `case_insensitive INTEGER`：授权时对该仓库根实测的大小写行为（探针文件写入后以变体大小写读取）；NULL 表示未测出，比较时按**敏感**处理。**所有平台都实测**，不按平台名假定——APFS 可格式化为大小写敏感卷，NTFS 也可按目录开启大小写敏感。
...
**输入校验先于比较**。`path.relative` 只回答"A 是否在 B 内"，不检查 A 本身是否是合法的仓库内相对前缀；把未净化的输入直接送进比较，`C:\x`、`\\server\share`、`../../etc` 都可能得出"看起来在范围内"的结果。校验分两步，**顺序不能颠倒**——上一轮把"拒绝绝对路径"和"`/` 归一成整仓"并列成两条规则，二者直接矛盾（`/` 就是绝对路径），实现方无从下手。

**第 0 步：整仓别名归一（在任何拒绝规则之前）。** 字面量 `""`、`"."`、`"/"` 三者都表示"整仓"，一律先归一成 `""`。这是唯一允许出现前导 `/` 的输入形式，且只接受**恰好一个字符**的 `"/"`；`"/src"` 不在此列，走下面的拒绝规则。

**第 1 步：对归一后非空的前缀逐条校验**，任一不通过即整体拒绝并返回 `SCOPE_INVALID_PREFIX`（不静默丢弃该条，否则用户以为限制生效了）：

1. 必须是相对路径：`path.posix.isAbsolute()` 为真即拒绝（此时 `"/"` 已在第 0 步消化）；同时显式拒绝 Windows 盘符（`^[A-Za-z]:`）与 UNC（`^\\\\`）——POSIX 的 `isAbsolute` 不认这两种。
2. 规范化后不得逃逸：`path.posix.normalize()` 后不得以 `..` 开头或等于 `..`。
3. 字符约束：拒绝 NUL（`\0`）与控制字符；拒绝反斜杠（统一用 `/`，避免同一目录两种写法绕过去重）。
4. 其余归一：去掉尾部 `/`、折叠重复 `/`、移除 `.` 段。
5. 大小写：**存原文**。比较语义**对所有平台一律实测，不按平台名假定**——APFS 可格式化成大小写敏感卷，NTFS 也可以按目录开启大小写敏感（`fsutil file setCaseSensitiveInfo`），所以"Windows 一定不敏感"同样是错的假设，会在这类目录上错误扩大授权范围。规则只有一条：授权时对该仓库根实测（写入探针文件再以变体大小写读取），结果记入 `repository_machine_paths.case_insensitive`；**实测不到（探测失败、只读挂载等）一律按敏感处理**。没有任何平台走"默认不敏感"。
...
- `access TEXT NOT NULL CHECK (access IN ('read_write','read_only'))`：**项目级访问意图**。机器级 `repository_machine_paths.access` 是"这台机器允许到什么程度"，项目级是"这个项目要用到什么程度"，两者取较严者。没有这一列，"reference 恒只读"就只是一句话——迁移写不进去，共享同一 Repository 的两个项目也无法一个可写、一个只读。
  - `CHECK (role = 'primary' OR access = 'read_only')`：`reference` 角色**在数据库层**恒为只读，不靠应用逻辑（spec 边界场景"参考仓库始终只读"）。
  - 与机器级的合成：`effective.access = read_write` 当且仅当两级都是 `read_write`；任一为 `read_only` 即只读。§5 的"参考仓库 `read_write` 请求硬拒绝"由上面的 CHECK 在写入时就拒掉，不用等到派工。
...
- `legacy_workspace_id TEXT REFERENCES workspaces(id)`：**Repository ↔ legacy workspace 的临时桥**，§7 的兼容投影靠它取到 `issues.workspace_id` 要写的值。桥放在这张表而不是 `repositories`，因为 `workspaces` 是 per-project 的（`(project_id, local_path_normalized)` 唯一），而 `repositories` 跨项目共享，挂在共享行上会串线。
  - **迁移时（不变量 D：一个 Project 至多一条 primary）**：`WorkspaceService.bind()` 保留历史 Workspace 行，v10 正式 fixture 里 `prj_v02_alpha` 就有两条（`ws_v02_alpha` 与一条 real-path workspace）。因此**不能把每条旧 workspace 都写成 `primary`**——那必然撞 `idx_project_primary_repo`。规则：
    1. `projects.default_workspace_id` 指向的那条 → `role='primary'`，`legacy_workspace_id` 回填；
    2. 同一 Project 的**其余** workspace → 各建 Repository 但 `role='reference'`（`access` 由上面的 CHECK 强制为 `read_only`），`legacy_workspace_id` 同样回填（历史 Run 挂在这些行上，需要可追溯）；
    3. `default_workspace_id` 为 NULL 且该 Project 无 workspace（fixture 的 `prj_v02_beta`）→ 不建任何引用；
    4. `default_workspace_id` 为 NULL 但存在 workspace（数据异常）→ 取 `created_at` 最早的一条作 primary，其余按第 2 条，并写一条 migration 诊断记录，不静默挑选。
  ...
  - **新建**的 `role='reference'` 行 `legacy_workspace_id` 恒为 NULL：参考仓库只读，不参与 v0.2 执行。迁移产生的 reference 行保留 `legacy_workspace_id` 仅供历史追溯——§7 的兼容投影**只读 `role='primary'` 那条**，因此不会把历史 workspace 误当作新任务的执行目录。
...
- `skills`：`id TEXT PRIMARY KEY`、`space_id TEXT REFERENCES spaces(id)`（NULL 表示内置 / 全局）、`display_name TEXT NOT NULL`、`source_kind TEXT NOT NULL CHECK (source_kind IN ('builtin','user','legacy-workflow'))`、`source_identity TEXT NOT NULL`、**`current_revision INTEGER NOT NULL`**（不变量 A）、**`state TEXT NOT NULL CHECK (state IN ('active','disabled'))`**——`conflict` 不在此列，它是 per-Space 结果，住在 `skill_space_state`（不变量 E）
...
`published_at IS NULL` 是**构建期**：可以写内容列、可以增删 `skill_revision_files`。激活事务内置上 `published_at` 后进入**冻结期**，此后任何修改都被拒。**五个发布态 trigger**各绑一种事件、各带发布态条件，互不重叠：
...
每个 `evidence_kind` 的摘要、预览、打开动作由注册表一次性定义；`EvidenceSpec` 只选 kind 与状态映射，不重新描述这三件事。这样 F012 拿到一条 completion requirement 就能确定适配器输出契约。
...
**`artifact` kind 的启用不由本文档描述。** 前几轮在这里反复出错：先是替 F010 编了一条它文档里没有的"`invalid`/`hash_mismatch` 归并为 `missing`"契约，接着又写"`EvidenceRefKind` 目前没有 artifact"——而 F010 分支当时已经加上了。**F013 不快照上游状态**：kind 的取值域**以 `server/src/evidence-ref.ts` 的 `EvidenceRefKind` 为唯一真相源**，实现时直接引用该类型，不在本文档另列清单。

需要澄清的是一条**范畴区别**（这条属于 F013 自己的判断，不会过期）：F010 §6 的 `loading` / `empty` / `ready` / `missing` / `invalid` / `hash_mismatch` 是**读取 UI 的 discriminated state**，与 `EvidenceResolution.status` 这个 **resolver 状态**不是同一层，两者不能直接对应。

**启用条件**：`artifact` 进入 `EvidenceRefKind` 后，还需要有人明确"Artifact 读取态 → `EvidenceResolution.status`"的归一化 owner（F010 或 F014）。该 owner 确定前，带 `evidence_kind: "artifact"` 的 requirement 激活时报 `SKILL_EVIDENCE_KIND_UNAVAILABLE`；判据是 `EvidenceRefKind` 的实际取值加上该归一化契约是否存在，**不是本文档里的一句状态描述**。

校验规则：`status_map` 三个键（`satisfied` / `failed` / `not_applicable`）必须齐全；取值必须落在 `resolved | missing | truncated` 内，越域拒绝激活（`SKILL_EVIDENCE_STATUS_UNKNOWN`）；同一状态不得出现在两个键下（互斥）。**三个状态必须被完整覆盖**：`resolved` / `missing` / `truncated` 各自恰好出现在一个键下，未覆盖即报 `SKILL_EVIDENCE_STATUS_UNMAPPED` 并**拒绝激活**。缺映射是配置错误，在激活时挡下，不静默放行。**某个 kind 能否用于激活，判据是它是否已认领归一化 owner**（见上文启用条件），不是本文档是否列出它——因此这里不写任何具体 kind 的可用性结论。
...
// 不在此处枚举 kind。取值域直接引用上游类型，上游新增成员时本处编译跟随：
//   import type { EvidenceRefKind } from "../evidence-ref";
// 某个 kind 能否用于激活，由「是否已认领归一化 owner」决定（见上文启用条件），
// 不由本文件里的一份清单决定。

type EvidenceSpec = {
  evidence_kind: EvidenceRefKind; // 上游类型，见上方注释
...
// 见 shared/src/types/trace.ts:113——不另造一套无生产者的状态名
type StatusMap = {
  satisfied: string[];
  failed: string[];
...
2. **检测（per-Space 分组，状态也必须 per-Space）**：全局 Skill（`space_id IS NULL`）在每个 Space 都可见，因此可能同时卷入多个 Space 的冲突组。**单值 `skills.state` 表达不了这件事**——上一轮写的"任一 Space 冲突即整体置 conflict"会让一个 Space 的同名问题殃及其它 Space，而 `resolve-conflict` 又没有 Space 参数，选择保留 global 时不知道在消解哪一组。因此拆成两层状态：

   - `skills.state` 只保留**全局意图**：`active` / `disabled`（用户是否禁用它），不再有 `conflict`。
   - 新增 `skill_space_state`：`(skill_id, space_id)` 主键 + `state TEXT NOT NULL CHECK (state IN ('active','shadowed','conflict'))`。它表达"该 Skill 在该 Space 的**生效结果**"。全局 Skill 在每个 Space 各有一行；Space 私有 Skill 只有自己那一行。
   - **行的物化时机必须明确，否则新建 Space 在下次 scan 前看不到任何全局 Skill**（缺行 = 查不到，而不是"默认可用"）。两个入口各自负责：
     - `SpaceService.create()` 在**同一事务**内为所有 `space_id IS NULL` 的 Skill 各插一行，初值按该 Space 内的分组结果算（新 Space 里只有全局 Skill，彼此同名才会是 `conflict`，否则 `active`）。
     - **物化只看分组结果，不代表可用**：全局 `disabled` 的 Skill 同样会得到一行 `active`——那是"它在这个 Space 里没被同名项遮蔽"的意思，不是"它能用"。
     - `SkillRegistry` 创建 / 激活 Skill 时，在同一事务内物化对应行：**global Skill（`space_id IS NULL`）为所有现存 Space 各一行；private Skill（`space_id = S`）只为 S 一行**。上一轮只写了 global 那半条，private Skill 因此没有任何物化路径——它创建后 `skill_space_state` 无行，而 eligibility 要求两层都为 `active`，结果是**自己 Space 里的 Skill 永远用不了**。
     - **写入一律用 UPSERT**：`INSERT INTO skill_space_state (...) VALUES (...) ON CONFLICT (skill_id, space_id) DO UPDATE SET state = excluded.state`。重复激活同一 Skill 是正常操作（改完再激活一次），直接 INSERT 会撞主键；UPSERT 同时保证"重新激活会按当前分组重算状态"，而不是保留上一次的 `shadowed`。
     - 读取端遇到缺行**按 `active` 之外的保守值处理**并记一条诊断，不静默当作可用；这是兜底，不是正常路径。
   - 分组：对 Space `S`，候选集 = `space_id = S` ∪ `space_id IS NULL`，按 `lower(display_name)` 分组。同组出现多个不同 `source_identity` 且 `skills.state='active'` 的成员 → 在同一事务内把**该组在 S 的那些行**置 `conflict`，写 `skill.conflict_detected`（payload 带 `space_id`）。其它 Space 的行不受影响。
   - **读取与 eligibility 必须同时满足两层**：`skills.state = 'active'`（全局意图）**AND** `skill_space_state.state = 'active'`（该 Space 的生效结果）。上一轮写成"一律看 `skill_space_state`，不看 `skills.state`"是错的——物化时全局 `disabled` 的 Skill 也会拿到一行 `active`，只看单层会让它在每个 Space 重新生效，用户的禁用被静默撤销。
   - 两层的分工是固定的：`skills.state` 回答"用户要不要它"，`skill_space_state` 回答"这个 Space 里它有没有被同名项挡住"。**任一为非 `active` 即不可用**，UI 分别显示"已禁用"与"被同名项遮蔽"，不合并成一个状态。

3. **消解**：`POST /api/skills/:id/resolve-conflict`，**请求必须带 `space_id`**——消解的是某个 Space 内的一组，不是全局。事务内把选中成员在该 Space 的行置 `active`、同组其余成员在该 Space 的行置 `shadowed`（被同名项遮蔽，来源仍在、其它 Space 不变）。选中 global Skill 只影响它在该 Space 的那一行，不动它在别处的生效状态。
4. **恢复**：当某 Space 的一组只剩一个候选成员时（另一方被全局禁用、来源消失或移出该 Space），扫描或消解动作把它在该 Space 的行自动置回 `active`；**不会有"冲突已消失但仍卡在 conflict"的悬挂状态**。恢复同样是 per-Space 的。
...
- **外键只管存在性，不管发布态**，且**按列监听的 trigger 挡不住所有写入路径**。两个实测反例：
  ① 存在 `skill_revisions('s1',2,published_at=NULL)` 时 `INSERT INTO project_skill_refs VALUES('p','s1',2)` 成功；
  ② 已有指向 `s1@2`(published) 的行，`UPDATE project_skill_refs SET skill_id='s2'` **不触发** `BEFORE UPDATE OF pinned_version`，若 `s2@2` 是 draft，该引用就静默落到 draft 上。
- 因此这里**不再逐条枚举 trigger**——前七轮每轮补一条规则、下一轮就被一条新路径绕过，说明"在文档里穷举 SQLite 写入路径"这件事本身不可靠。改为声明不变量并把完备性交给测试：
  > **不变量 A（总）**：`project_skill_refs` 与 `skills` 的**任何**写入路径结束后，每一条引用都必须能解析到一条 `published_at IS NOT NULL` 的 revision。
  > 实现上至少需要覆盖 INSERT、UPDATE（**任一相关列，包括只改 `skill_id`**）、`INSERT OR REPLACE`、UPSERT 四类路径；具体用几个 trigger、是否改用 `BEFORE UPDATE ON`（不带 `OF`）由实现决定，**判据是 T011 的穷举测试全绿**，不是文档里列了几条。
...
- `skills.current_revision` **只能指向 `published_at IS NOT NULL` 的 revision**，由**数据库**保证而非"激活事务自觉"——Round 4 的反例正是 `current_revision=1` 指向 `published_at=NULL` 的 revision 可以提交。两个 trigger 各管一端：
  - 实现约束（实测）：SQLite 的 `CREATE TRIGGER` 只能绑定单一事件，`BEFORE INSERT OR UPDATE OF ...` 直接报 `near "OR": syntax error`；`BEFORE UPDATE OF <列>` 只在该列出现在 SET 子句里才触发，因此**按列监听挡不住"只改另一列"的写法**。这两条决定了实现必须覆盖多个 trigger 或改用不带 `OF` 的 `BEFORE UPDATE`，但**具体形态由实现选择，判据是 T011 的穷举测试**（见下方不变量 A）。
...
`CURRENT_SCHEMA_VERSION` 当前为 11，F010 与 F013 并行且都要顺延。**两者都不预占版本号**：实施时读取仓库当前值 +1；先合入者取得该号，后合入者 rebase 后重新编号并改 migration 文件名。禁止在文档或 tasks 中写死 `v12`。Migration 除上述 `projects` 与 `issues` 两张表的 rebuild 外只追加表 / 索引；仓库没有 down migration，不声明不可执行的「回滚前检查」。
...
| `AC-001` | integration        | `server/tests/integration/legacy-workspace-migration.test.ts`                                       | **v10 fixture 的 `prj_v02_alpha`（两条 workspace）升级后只产生一条 `role='primary'`**，另一条为 `role='reference'` 且 `access='read_only'`、两者 `legacy_workspace_id` 均已回填；`prj_v02_beta`（无 workspace）不产生引用；`default_workspace_id` 为 NULL 但有 workspace 的构造数据取最早一条并留下诊断记录；`idx_project_primary_repo` 全程无冲突                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| `AC-001` | integration        | `server/tests/integration/legacy-compat-projection.test.ts`                                         | 升级后经 `IssueService` 创建带 Project 的任务，三列仍按兼容投影写入且 v0.2 执行链路可跑通；**F013 之后新建的 Project 绑定 primary 仓库时自动 upsert legacy workspace 行**，其任务同样可进入执行链；改绑 primary 后投影指向新行；**新建**的 reference 角色 `legacy_workspace_id` 恒为 NULL（迁移产生的保留，仅供追溯）；投影只读 primary 行；游离任务三列为空且不进入该链路                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| `AC-002` | unit + integration | `server/tests/unit/scope-validation.test.ts`、`server/tests/integration/authorization-recheck.test.ts`、`server/tests/integration/legacy-compat-projection.test.ts` | symlink / junction 越界拒绝、大小写路径、`path.relative` 边界（`/a/bc` 不在 `/a/b` 内、`src/ab` 不在 `src/a` 内）、参考仓库 `read_write` 硬拒绝、项目范围只能收紧、旧 workspace 迁移不产生已授权 `real_path` **`role='reference'` 行写 `access='read_write'` 被 CHECK 拒绝**（不是应用层拦截）；项目级与机器级 access 取较严者；同一 Repository 被两个项目分别以 read_write / read_only 引用时互不影响                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| `AC-002` | unit               | `server/tests/unit/scope-validation.test.ts`                                                        | **`""` / `"."` / `"/"` 三个整仓别名在拒绝规则之前归一成 `""`**（证明第 0 步先于第 1 步执行），而 `"/src"` 仍按绝对路径拒绝；其余五类非法前缀逐一被拒并返回 `SCOPE_INVALID_PREFIX`——`C:\x` 盘符、`\\server\share` UNC、`../..` 逃逸、含 NUL / 控制字符、含反斜杠；同值与被包含前缀去重保留较短者；**大小写：所有平台都以 `case_insensitive` 实测值为准，NULL 一律按敏感**（构造大小写敏感的 fixture 断言不会错误放宽；**含一个 Windows 侧大小写敏感目录的用例**，证明没有走平台名分支）；`write` 不在 `read` 内时返回 `SCOPE_WRITE_NOT_IN_READ`                                                                                                                                                                                                                                                                                                                                                                    |
...
| `AC-002` | integration        | `server/tests/integration/project-lifecycle.test.ts`                                                | archive 后默认列表不含该项目、按 ID 深链仍可读、历史任务证据可读；归档态下新建 Issue / 改绑仓库 / 改默认 Skill 均返回 `PROJECT_ARCHIVED`；restore 后恢复；**六类引用逐一构造后 DELETE 均返回 `PROJECT_HAS_REFERENCES` 并列出正确类别**（issues / workspaces / agent_configs / intake_confirmations / project_repository_refs / project_skill_refs）；六类全空后可删；**用 `PRAGMA foreign_key_list` 反查所有指向 projects 的表并与检查清单比对，漏项即失败**                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
| `AC-004` | unit               | `server/tests/unit/evidence-spec.test.ts`                                                           | `evidence_kind` 的类型来自上游 `EvidenceRefKind`——**给该类型新增一个成员，本处编译即跟随**（断言 F013 侧没有第二份 kind 清单）；`status_map` 取值须落在 `resolved`/`missing`/`truncated` 内，越域即 `SKILL_EVIDENCE_STATUS_UNKNOWN`（含误用的 `emitted`/`present`/`empty` 作为负例）；三键齐全互斥且**三个状态各恰好出现一次，未完整覆盖即 `SKILL_EVIDENCE_STATUS_UNMAPPED` 并拒绝激活**（不是告警后按 failed 放行）；**未认领归一化 owner 的 kind 报 `SKILL_EVIDENCE_KIND_UNAVAILABLE`**（判据是 owner 是否存在，不是文档里写没写该 kind）；registry 的 summary/preview/open 由 kind 决定，要求侧不可覆写                                                                                                                                                                                                                                                                                                        |
...
| `AC-003` | integration        | `server/tests/integration/skill-default-ref.test.ts`                                                | 一个项目至多一条 `is_default=1`；**按 T011 穷举四类写入路径**（INSERT / UPDATE 逐列含只改 `skill_id` / `INSERT OR REPLACE` / UPSERT），每类结束后引用都解析到 `published_at IS NOT NULL` 的 revision——已知反例：ghost Skill + NULL pinned、`current_revision` 写 NULL、删除被引用的 revision、pinned 指向 draft、只改 `skill_id` 换到含 draft 同版本号的 Skill；正例：disabled Skill 的默认 ref 仍解析到真实且已发布的 revision                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| `AC-005` | integration        | `server/tests/integration/skill-space-boundary.test.ts`                                             | 引用另一 Space 的 Skill 被 trigger 拒绝（`SKILL_SPACE_MISMATCH`），INSERT 与 UPDATE 都覆盖；引用全局 Skill 允许；**全局 Skill 与 Space 内同名 Skill 会被分到同一组并双双在该 Space 置 `conflict`**（不是各自生效）；**global Skill 在 Space A 冲突不影响它在 Space B 的 `active`**；`resolve-conflict` 不带 `space_id` 时拒绝；选中 global 保留只改它在该 Space 的行 **先建全局 Skill、后建 Space**：新 Space 创建后立即查询即可看到该 Skill（行已在创建事务内物化，不依赖下次 scan）；先建 Space 后建全局 Skill 同样为所有现存 Space 物化行 **全局 `disabled` 的 Skill 在新建 Space 后仍不可用**（物化行为 `active` 但 `skills.state='disabled'`，两层与运算为假）——这是 Round 5 的反例；UI 区分"已禁用"与"被遮蔽"两种不可用原因 **private Skill 创建后在自己 Space 有行且可用**（Round 6 反例：只物化 global 时它永远用不了）；**重复激活同一 Skill 不报主键冲突**且状态按当前分组重算，不保留上次的 `shadowed` |
| `AC-005` | integration        | `server/tests/integration/skill-conflict.test.ts`                                                   | 同名双来源**双方**都进入 `conflict` 且都不生效；`resolve-conflict` 必须带 `space_id`；该 Space 内保留方 `skill_space_state='active'`、同组其余 `shadowed`（**不是 `disabled`**——那是全局意图，不是 per-Space 结果）；某 Space 的一组只剩一个候选时该 Space 的行自动回 `active`（无悬挂 conflict）；同一 `source_identity` 重扫是更新不是新建；非法 steps schema / 保留 ID / 无来源在激活前拒绝；重启后冲突状态可见                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| Skill 归属形状               | `skills.space_id` 一对多，取代规划期的 `space_skills` 关联表（用户裁决 2026-09-12）                                                           | spec §3「范围外」明确排除跨 Space 共享 Skill（PRD §15：预留没有消费者的字段就是下次要删的东西）。**注意裁决当时给的另一条理由"v0.3 只有一个 Space"已不成立**——本设计支持 create / select 多 Space，`conflict` 也确实成了 per-Space 状态（`skill_space_state`）。但裁决结论不变：一对多管的是**所有权**，per-Space 状态管的是**某个 Space 内的生效结果**，两者正交；多对多要表达的是跨 Space 共享同一个 Skill，那仍在范围外 | 未来共享叠加 `skill_visibility` 表，纯追加 migration；见 §3 与 DQ-001 |
| 同名冲突的表达               | 不用唯一约束，由 SkillRegistry 事务内检测并把双方在**该 Space** 的 `skill_space_state` 行置 `conflict`                                        | 唯一约束会让第二来源插入失败，无法满足"两者都不生效且状态可见"                                                                                                                                                                                                                                                                                                                                                             | —                                                                     |

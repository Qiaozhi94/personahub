...
- **外键只管存在性，不管发布态**——实测反例：`skill_revisions('s1',2,published_at=NULL)` 存在时，`INSERT INTO project_skill_refs VALUES('p','s1',2)` 成功，pinned ref 指向了一条 draft revision，"每个引用都解析到已发布 revision"随即破功。因此 `pinned_version` 也要两个即时 trigger（同样受 SQLite 单事件限制）：
  - `trg_pinned_published_ins`：`BEFORE INSERT ON project_skill_refs`
  - `trg_pinned_published_upd`：`BEFORE UPDATE OF pinned_version ON project_skill_refs`

  体相同：`WHEN NEW.pinned_version IS NOT NULL AND (SELECT published_at FROM skill_revisions WHERE skill_id = NEW.skill_id AND version = NEW.pinned_version) IS NULL` 时 `RAISE(ABORT,'SKILL_PINNED_NOT_PUBLISHED')`。`pinned_version` 为 NULL 时跳过（那条路径由 `skills.current_revision` 的 trigger 保证）。

...
- [x] T002 (`FR-001`, `FR-002`, `NFR-001`): 增加顺延 migration、Repository、`(artifact_id, idempotency_key)` 唯一约束、source/archive locator 分离、maintenance lease 与不变量测试；证明 forward-only 升级不破坏既有查询。 — verify: `npm test --workspace server`
- [x] T003 (`FR-003`, `NFR-002`): 在 `server/src/evidence-ref.ts` 实现统一 ref parser / builder 的可选 revision，增加 `resolveForRead` / `resolveForDispatch`，覆盖路径、SHA-256 与 floating ref 拒绝。 — verify: `npm test --workspace server`
...
- [x] T011 (`FR-004`, `FR-005`): 提供幂等 `recordConsumption(dispatch_id, run_id, revision_ref)` 公共契约与双向 provenance 查询；不接入尚未存在的上下文组装器。 — verify: `npm test --workspace server`
- [x] T012 [P] (`FR-005`): 增加 API client、共享类型与只读 hooks，区分 loading / empty / ready / missing / invalid / hash mismatch；不注册 `SurfaceRegistry` 槽位、不交付可见组件。 — verify: `npm test --workspace web`

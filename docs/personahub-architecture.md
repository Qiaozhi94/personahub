...
| 日期 | 来源提交 | 修订目的 | 修订内容 |
| --- | --- | --- | --- |
| 2026-08-31 | `docs/decisions/0015-daemon-readiness-fields.md` | 核查三个参考项目的 CLI 对接方式后确认 PersonaHub 与 clowder-ai 同为本机 spawn、无 daemon；`AdapterConfig` 与 `AdapterIdentitySnapshot` 均无执行位置维度，而这两处的历史记录**无法回填** | 第 2 节 v0.1→v0.7 对照表新增「执行位置」一行，说明 `runtime_id` 字段已按 ADR 0015 提前留好且恒为 `'local'`；「Workspace 锁」一行补充说明 ADR 0015 只把「新进程启动 = 旧进程已死」收成具名判定函数，owner fencing 本身仍是候选设计 |
...
| 执行位置 | `AdapterConfig.runtime_id` 与 `AdapterIdentitySnapshot.runtime_id` 恒为 `'local'`，无行为差异 | 字段已按 `docs/decisions/0015-daemon-readiness-fields.md` 提前留好，因为 Run 与 evidence summary 里的历史记录**无法回填**；远程执行本身仍按 ADR 0008 第 3 条的触发信号推进 |
| Workspace 锁 | 锁状态存 DB，按 `workspace_id` 原子获取；启动时恢复遗留 running Run/锁，无 lease/heartbeat | v0.7 多实例需要新增 owner fencing（候选：`runner_instance_id` + lease/heartbeat），不能直接假设当前单进程锁可无修改扩展。ADR 0015 只把「新进程启动 = 旧进程已死」这条推断收成一个具名判定函数，fencing 本身仍是候选设计 |

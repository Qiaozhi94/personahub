...
describe("Database Migration", () => {
  ...
  it("creates all 14 tables", () => {
    ...
    expect(tableNames).toContain("graph_runs");
    expect(tableNames).toContain("node_runs");
  });

  it("seeds default coding workflow template", () => {
    applyMigrations(db);
    const row = db.prepare("SELECT * FROM workflow_templates WHERE id = ?").get("wft_coding_default") as Record<
      string,
      unknown
    >;
  ...
  it("seeds default coding validation policy", () => {
    applyMigrations(db);
    const row = db.prepare("SELECT * FROM validation_policies WHERE id = ?").get("vpl_coding_default") as Record<
      string,
      unknown
    >;
    expect(row).toBeDefined();
  ...
  it("creates partial unique index for primary threads", () => {
    applyMigrations(db);
    const indexes = db
      .prepare("SELECT name FROM sqlite_master WHERE type='index' AND name='idx_threads_primary_unique'")
      .all() as { name: string }[];
  ...
  it("creates index on thread_events(thread_id, event_sequence)", () => {
    applyMigrations(db);
    const indexes = db
      .prepare("SELECT name FROM sqlite_master WHERE type='index' AND name='idx_thread_events_thread_seq'")
      .all() as { name: string }[];
    expect(indexes).toHaveLength(1);
  });
  ...
  it("creates unique index on workspaces(project_id, local_path_normalized)", () => {
    applyMigrations(db);
    const indexes = db
      .prepare("SELECT name FROM sqlite_master WHERE type='index' AND name='idx_workspaces_project_path'")
      .all() as { name: string }[];
    expect(indexes).toHaveLength(1);
  });
  ...
  it("v2 to v3 upgrade preserves existing data", () => {
    ...
    const tableCount = freshDb
      .prepare("SELECT COUNT(*) as c FROM sqlite_master WHERE type='table' AND name='run_trace_states'")
      .get() as { c: number };
  ...
  it("v4 creates evidence_summaries table", () => {
    applyMigrations(db);
    const tables = db
      .prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='evidence_summaries'")
      .all() as { name: string }[];
  ...
  it("v4 creates active validator partial unique index", () => {
    applyMigrations(db);
    const indexes = db
      .prepare("SELECT name FROM sqlite_master WHERE type='index' AND name='idx_runs_one_active_validator'")
      .all() as { name: string }[];
    expect(indexes).toHaveLength(1);
  });
  ...
  it("v4 creates runs issue_role_created index", () => {
    applyMigrations(db);
    const indexes = db
      .prepare("SELECT name FROM sqlite_master WHERE type='index' AND name='idx_runs_issue_role_created'")
      .all() as { name: string }[];
    expect(indexes).toHaveLength(1);
  });
  ...
  it("v3 to v4 upgrade gives old runs default role and dispatch_source", () => {
    ...
    db.prepare(
      "INSERT INTO workspaces (id, project_id, local_path, local_path_normalized, lock_state, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?)",
    ).run("wsp_t", "prj_t", "/tmp", "/tmp", "idle", now, now);
    db.prepare(
    ...
    ).run("agc_t", "prj_t", "Fake", "implementation", "fake", "fake", "[]", "[]", "available", now, now);
    db.prepare(
      "INSERT INTO issues (id, project_id, workspace_id, issue_type, workflow_template_id, validation_policy_id, title, status, priority, labels, validation_round_count, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
    ).run(
      "iss_t",
    ...
    db.prepare(
      "INSERT INTO threads (id, issue_id, thread_type, title, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?)",
    ).run("thr_t", "iss_t", "primary", "T", now, now);
    db.prepare(
      "INSERT INTO runs (id, issue_id, thread_id, workspace_id, adapter_config_id, status, instructions, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
    ).run("run_t", "iss_t", "thr_t", "wsp_t", "agc_t", "completed", "do", now, now);
  ...
  it("v3 to v4 upgrade preserves existing issue data", () => {
    ...
    db.prepare(
      "INSERT INTO workspaces (id, project_id, local_path, local_path_normalized, lock_state, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?)",
    ).run("wsp_t2", "prj_t2", "/tmp2", "/tmp2", "idle", now, now);
    db.prepare(
      "INSERT INTO issues (id, project_id, workspace_id, issue_type, workflow_template_id, validation_policy_id, title, goal, status, priority, labels, validation_round_count, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
    ).run(
      "iss_t2",
      "prj_t2",
      "wsp_t2",
  ...
  it("v4 seed updates steps_json with schema_version and steps", () => {
    applyMigrations(db);
    const row = db.prepare("SELECT steps_json FROM workflow_templates WHERE id = ?").get("wft_coding_default") as {
      steps_json: string;
    };
  ...
  it("v4 seed updates evidence_requirements_json with schema_version and requirements", () => {
    applyMigrations(db);
    const row = db
      .prepare("SELECT evidence_requirements_json FROM validation_policies WHERE id = ?")
      .get("vpl_coding_default") as { evidence_requirements_json: string };
  ...
  it("v4 seed does not overwrite custom evidence_requirements_json", () => {
    ...
    const customReq = JSON.stringify({
      ...
      require_file_trace: false,
      require_verification: false,
      accepted_verification_kinds: [],
    });
    db.prepare("UPDATE validation_policies SET evidence_requirements_json = ? WHERE id = ?").run(
      customReq,
      "vpl_coding_default",
    );
  ...
  it("v4 active validator unique index prevents duplicate queued validators", () => {
    ...
    db.prepare(
      "INSERT INTO workspaces (id, project_id, local_path, local_path_normalized, lock_state, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?)",
    ).run("wsp_u", "prj_u", "/tmp", "/tmp", "idle", now, now);
    db.prepare(
      "INSERT INTO agent_configs (id, project_id, name, role, cli_provider, command, args, capability_tags, status, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
    ...
    db.prepare(
      "INSERT INTO threads (id, issue_id, thread_type, title, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?)",
    ).run("thr_u", "iss_u", "primary", "U", now, now);
    db.prepare(
      "INSERT INTO runs (id, issue_id, thread_id, workspace_id, adapter_config_id, status, instructions, role, dispatch_source, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
    ).run("run_v1", "iss_u", "thr_u", "wsp_u", "agc_u", "queued", "validate", "validator", "system", now, now);
    ...
    expect(() =>
      db
        ...
        .run("run_v2", "iss_u", "thr_u", "wsp_u", "agc_u", "running", "validate", "validator", "system", now, now),
    ).toThrow();
  });
});

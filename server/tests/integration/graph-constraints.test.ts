...
function seedDb(db: Database.Database) {
  ...
  db.prepare("INSERT INTO projects (id, name, space_id, created_at, updated_at) VALUES (?, ?, (SELECT id FROM spaces WHERE is_default = 1), ?, ?)").run("prj_1", "test", now, now);
  ...
  db.prepare("INSERT INTO issues (id, space_id, project_id, workspace_id, issue_type, workflow_template_id, validation_policy_id, title, status, priority, labels, validation_round_count, created_at, updated_at) VALUES (?, (SELECT id FROM spaces WHERE is_default = 1), ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)").run("iss_1", "prj_1", "wsp_1", "coding", "wft_1", "vpl_1", "test", "Inbox", "normal", "[]", 0, now, now);
  db.prepare("INSERT INTO issues (id, space_id, project_id, workspace_id, issue_type, workflow_template_id, validation_policy_id, title, status, priority, labels, validation_round_count, created_at, updated_at) VALUES (?, (SELECT id FROM spaces WHERE is_default = 1), ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)").run("iss_2", "prj_1", "wsp_1", "coding", "wft_1", "vpl_1", "test2", "Inbox", "normal", "[]", 0, now, now);

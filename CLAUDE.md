...
F005（Manual Multi-Agent Routing，新增 Claude Code / OpenCode adapter 与手动路由）Phase 0-10（共 13 个 Phase，T001-T082）已完成：三 provider（Codex/Claude/OpenCode）真实 CLI probe 与 adapter 落地、schema v6、secret-safe DTO、`ManualRoutingService` 统一 Run 创建入口（purpose/role 分类、AdapterResolver、RunContextBuilder handoff 上下文、consult 状态边界与安全优先 escalation）、Validator Grace 两阶段 dispatch（Phase A `validation.dispatch_pending` 冻结上下文 + Phase B `claimValidatorSlot()` 互斥创建，`ValidationDispatchScheduler` 到期自动派发，手动 explicit adapter 与 scheduler 的 active/per-round 冲突分流，restart recovery 按 `validation_dispatch_due_at` 重写）、HTTP API 收尾（adapter CRUD 全字段路由、default-adapter PUT、`GET /api/adapter-providers` metadata、canary secret 扫描零泄漏、SSE replay routing metadata 完整性），typecheck、server（1277 测试）/web（78 测试）自动化测试和生产构建均通过，状态为 `in-progress`。剩余 Phase 11-12（Adapter Settings/Composer/Thread/Inspector UI）、Phase 13（安全/端到端/文档回写）待续，详见 `docs/features/0.1/F005-multi-agent-manual-routing/tasks.md`。

...
function verifyForbiddenMutation(documents, phrase) {
  assert.throws(() => forbidPhrases([...documents, phrase], [phrase]), /forbidden v0\.3 planning contract/);
}
...
function verifyBrowserCheckApplicability(catalog) {
  ...
  const counts = rows.reduce((result, row) => ({ ...result, [row[2]]: result[row[2]] + 1 }), {
    adapted: 0,
    deferred: 0,
    "not-applicable": 0,
  });
...
const completionEvidencePlaceholders = new Set(["todo", "tbd", "pending", "placeholder", "待补", "—"]);
...
function verifyMigrationMatrixProgress(matrix) {
  ...
  assert.deepEqual(
    rows.map((row) => row[0]),
    expectedIds,
    "migration row ids must be unique and contiguous",
  );
...
function verifyDraftGenerationContract(documents) {
  ...
  for (const phrase of phrases) {
    assert.ok(normalized.includes(phrase.replace(/\s+/g, "")), `missing v0.3 planning contract: ${phrase}`);
  }
  assert.doesNotMatch(
    normalized,
...
test("F009-DOC-R6-010: migration targets and implementation progress are independently tracked", () => {
  ...
  const blankStatus = replaceP001Row(matrix, P001_COMPLETED_ROW.replace("| migrated | migrated |", "| migrated | |"));
  ...
  assert.throws(() => verifyMigrationMatrixProgress(forgedCompletion), /must remain pending before implementation/);
  ...
  assert.throws(
    () => verifyMigrationMatrixProgress(missingEvidence),
    /missing completion evidence|placeholder completion evidence/,
  );
});
...
test("F009-DOC-R7-013: completed migration evidence has non-empty disposition-specific values", () => {
  ...
  for (const placeholder of ["TODO", "tBd", "PeNdInG", "placeholder", "待补", "—"]) {
    ...
    assert.throws(() => verifyMigrationMatrixProgress(placeholderValue), /placeholder completion evidence value/);
  }
  ...
  assert.throws(() => verifyMigrationMatrixProgress(placeholderDeferredOwner), /placeholder completion evidence value/);
...
test("F009-DOC-R1-006: all 125 V3.44 browser checks have an explicit disposition", () => {
  ...
  const implementationNotes = read("ui-reference/personahub-draft/personahub-v3.1/browser-check.mjs");
  ...
  const removeFirstCatalogRow = (input) => input.replace(/^\| BC-001 \|[^\r\n]*(?:\r?\n|$)/m, "");
  ...
  assert.throws(() => verifyBrowserCheckApplicability(crlfMutated), /expected 125 classified browser checks/);
  assert.notEqual(
...
function classifyDeferredBrowserChecks(catalog) {
  ...
  const covered = new Set([
    ...
    ...NAV_ABSENT,
    ...CONTENT_CONTRACT,
    ...SETTINGS_CATALOG_CLOSED,
    ...COMPOSER_ABSENT,
    ...RUNTIME_PAGE_ABSENT,
    ...ADAPTER_DETAIL_ABSENT,
    ...WORKSPACE_BINDING_MINIMAL,
    ...TASK_CREATE_MINIMAL,
  ]);
...
test("F009-CODE-DEFERRED-INVENTORY: all 96 deferred browser checks are accounted for — provably unreachable, explicitly not-applicable, or a named residual", () => {
  ...
  for (const name of ["会话", "自动化", "记忆", "能力", "统计"]) {
    assert.ok(shell.includes(`"${name}"`), `BC-070 case must still assert "${name}" has zero primary-nav buttons`);
  }
  assert.match(
    journey,
    /task-unsupported-view|unsupported-view/,
    "S1 must still assert the task-view route family is unreachable",
  );
  ...
  for (const bcId of [
    "BC-027/028/040/058",
    "BC-033",
    "BC-054/082/087/088/090/101/116",
    "BC-108/109",
    "BC-102/107/112",
  ]) {
  ...
  assert.throws(() => {
    assert.ok(
      shellWithoutMemoryAssertion.includes('"记忆"'),
      'BC-070 case must still assert "记忆" has zero primary-nav buttons',
    );
...
test("F011-DOC-R1-D015: every F011::Txxx applicability pointer resolves to a defined F011 task", () => {
  ...
  assert.deepEqual(dangling.sort(), [], `F011 applicability points at undefined task id(s): ${dangling.join(", ")}`);
...
test("F011-DOC-R2-004: each browser check points at the expected F011 task, not just a defined one", () => {
  ...
  assert.throws(
    () => assert.deepEqual([...mutated.get("BC-003")].sort(), [...F011_APPLICABILITY_EXPECTED["BC-003"]].sort()),
